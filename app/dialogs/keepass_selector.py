"""Dialog for browsing and selecting a KeePass entry from the active database.

Shows all entries from the currently open KeePass database in a flat,
searchable list.  The caller reads ``dialog.selected_entry`` after
``exec()`` returns ``Accepted``.

Usage::

    dlg = KeePassSelectorDialog(parent=self)
    if dlg.exec() == QDialog.DialogCode.Accepted:
        entry = dlg.selected_entry   # pykeepass Entry object or None

Filter matches against entry title, username, and group name
(case-insensitive).
