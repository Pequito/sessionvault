"""Dialog for creating or editing a session of any supported protocol.

Tabs
----
General
    Session name, protocol, hostname, port, username, folder, private key.
    Protocol selection auto-updates the default port.

SSH Options
    X11 forwarding checkbox.
    Local port-forward tunnel list with Add / Remove actions.
    Each tunnel is defined via the embedded :class:`_TunnelDialog`.

RDP Options
    Session resolution (width × height) and full-screen toggle.
    Note: VNC options are minimal; the external viewer handles its own config.

KeePass Credential section
    Link a KeePass entry whose credentials are supplied automatically when
    connecting without prompting for a password.

The completed :class:`~app.models.SSHSessionConfig` is available as
``dialog.result_session`` after ``exec()`` returns ``Accepted``.
