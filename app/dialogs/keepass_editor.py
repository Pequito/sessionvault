...
-------
KeePassEntryDialog
    Create or edit a single KeePass entry (title, username, password, URL,
    notes, group).  Password field has an inline show/hide toggle.
    Calls ``keepass_manager.add_entry()`` or ``update_entry()`` on save.

KeePassNewDatabaseDialog
    Create a brand-new ``.kdbx`` file.  The user selects a save path,
    enters and confirms the master password, and chooses between:
    - KDBX4  (ChaCha20 + Argon2 — recommended, most secure)
    - KDBX3  (AES-256 + PBKDF2 — legacy compatibility)
    Calls ``keepass_manager.create_database()`` on accept.
