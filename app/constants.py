"""Application-wide constants: file paths, metadata, and theme color palettes.

Constants
---------
APP_NAME        Human-readable application name ("SessionVault").
APP_VERSION     Current semantic version string.
DATA_DIR        Root data directory: ~/.sessionvault/
SESSIONS_FILE   JSON file storing all saved sessions.
SETTINGS_FILE   JSON file storing user preferences.
MACROS_FILE     JSON file storing named command macros.
PLUGINS_DIR     Directory scanned for user .py plugin files.
LOGS_DIR        Directory for rotating log files.

THEMES          Ordered list of built-in theme names.
THEME_PALETTES  Dict mapping theme name → color key → hex string.
                Color keys: base, mantle, crust, surface0, surface1,
                overlay1, text, blue, green, red, yellow, mauve, pink.

ANSI_*          ANSI/VT color constants used by the terminal widget.
