...
class KeePassManager:
    ...
    def __init__(self) -> None:
        ...
        self._known_paths: list[str] = []        # paths seen this session (survive lock)
    ...
    def known_paths(self) -> list[str]:
        """All paths seen this session – includes locked (closed) databases."""
        with self._lock:
            return list(self._known_paths)

    def is_path_locked(self, path: str) -> bool:
        """True when *path* is known but not currently open (i.e. locked)."""
        with self._lock:
            return path in self._known_paths and path not in self._dbs

    def register_known_paths(self, paths: list[str]) -> None:
        """Pre-register paths as known-but-locked (e.g. restored from settings).

        Called at startup so the panel can show last-session databases as
        locked entries before the user enters their master password.
        Only registers paths that exist on disk.
        """
        import pathlib  # noqa: PLC0415
        with self._lock:
            for p in paths:
                if p not in self._known_paths and pathlib.Path(p).exists():
                    self._known_paths.append(p)
                    log.debug("Registered known (locked) database: %s", p)
    ...
    def lock(self) -> None:
        """Clear all in-memory databases while keeping paths in known_paths."""
        with self._lock:
            count = len(self._dbs)
            # Preserve known_paths so the panel can still list them as locked
            for p in self._dbs:
