...
-------
MacroManagerDialog
    Modal dialog listing all saved macros.  Double-clicking (or pressing
    "Play") calls the ``on_play`` callback with the list of command strings
    and closes the dialog.  Supports rename and delete.

MacroSaveDialog
    Minimal dialog that prompts for a macro name after recording is
    stopped.  Calls ``macro_manager.save_macro()`` on accept.
